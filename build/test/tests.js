...
function getTests(testDirPath) {
    const res = [
        ...
        },
        {
            ...
            onlyAsync: true,
            runAsync: () => {
                return (0, src_1.compare)(testDirPath + '/d1', testDirPath + '/none', {})
        ...
        },
        {
            ...
            print(cmpres, writer) {
                ...
                writer.write(' diffset: ' + JSON.stringify((_a = cmpres.diffSet) === null || _a === void 0 ? void 0 : _a.sort(comparator), null, 0));
            }
        },
        ////////////////////////////////////////////////////
        // Compare date                                   //
...
function customNameCompare(name1, name2, options) {
    ...
    if (options.ignoreExtension) {
        name1 = path_1.default.basename(name1, path_1.default.extname(name1));
        name2 = path_1.default.basename(name2, path_1.default.extname(name2));
    }
    return ((name1 === name2) ? 0 : ((name1 > name2) ? 1 : -1));
}
function printRelativePathResult(res, testDirPath, writer) {
    let result = res.diffSet.map(diff => util.format('path1: %s, path2: %s', diff.path1, diff.path2));
    result = JSON.stringify(result);
    result = result.replace(/\\\\/g, "/");
    result = result.replace(new RegExp(testDirPath.replace(/\\/g, "/"), 'g'), 'absolute_path');
    writer.write(result);
}
function validateSymlinks(expected, actual) {
