 ...
 */
export = async function compare(rootEntry1: OptionalEntry, rootEntry2: OptionalEntry, level: number, relativePath: string,
    ...
    while (i1 < entries1.length || i2 < entries2.length) {
        ...
        if (cmp === 0) {
            ...
            if (permissionDeniedState === "access-ok") {
                const compareEntryRes = await entryEquality.isEntryEqualAsync2(entry1, entry2, type1, options)
            ...
            if (!options.skipSubdirs && type1 === 'directory') {
                await compare(entry1, entry2, level + 1, pathUtils.join(relativePath, entry1.name), options, statistics, diffSet, loopDetector.cloneSymlinkCache(symlinkCache))
            }
        } else if (cmp < 0) {
            // Right missing
            ...
            if (type1 === 'directory' && !options.skipSubdirs) {
                await compare(entry1, undefined, level + 1, pathUtils.join(relativePath, entry1.name), options, statistics, diffSet, loopDetector.cloneSymlinkCache(symlinkCache))
            }
        } else {
            // Left missing
            ...
            if (type2 === 'directory' && !options.skipSubdirs) {
                await compare(undefined, entry2, level + 1, pathUtils.join(relativePath, entry2.name), options, statistics, diffSet, loopDetector.cloneSymlinkCache(symlinkCache))
            }
        }
    }
}
